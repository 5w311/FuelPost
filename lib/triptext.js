...
function formatTripText(trip) {
  const {
    pickupAddr, deliveryAddr, maxRange, rangeAtPickup,
  ...
  if (rangeAtPickup != null && rangeAtPickup !== maxRange) {
    lines.push(`Range: fuel before ${maxRange} mi  |  ${rangeAtPickup} mi leaving the shipper`);
  } else {
    lines.push(`Range: fuel before ${maxRange} mi`);
  }
  lines.push('');
