...
function planAdaptive(poly, routeMiles, stops, maxRange, startBurned, tiers = [8, 15, 30], arrivalReserve = 0, targetLeg = 0, skipShortFinal = null) {
  let last = null;
  for (const tol of tiers) {
    const proj = projectStops(poly, stops, tol);
    const r = planFuel(routeMiles, proj, maxRange, startBurned, arrivalReserve, targetLeg,
                       skipShortFinal);
...
function planBeyondGap(poly, routeMiles, stops, maxRange, gap, detourMax, arrivalReserve = 0, targetLeg = 0, skipShortFinal = null) {
  ...
  // The remainder still ends at the delivery, so the reserve applies to it
  // exactly as it does on an ungapped run.
  const rest = planFuel(routeMiles - anchor.mile, shifted, maxRange, 0, arrivalReserve,
                        targetLeg, skipShortFinal);
  const plan = [
  ...
  return {
    ...
    // Shifted back like every other mile above: the remainder was planned with
    // the anchor at mile 0, and a skipped stop is reported to the driver
    // against the whole route. milesFromDelivery needs no shift — it is
    // measured from the end, which both frames share.
    droppedFinal: rest.droppedFinal
      ? { ...rest.droppedFinal, mile: +(rest.droppedFinal.mile + anchor.mile).toFixed(1) }
      : null,
