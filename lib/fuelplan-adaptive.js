...
// arrivalTarget rides through the same way arrivalReserve does (v1.38.0).
function planAdaptive(poly, routeMiles, stops, maxRange, startBurned, tiers = [8, 15, 30], arrivalReserve = 0, arrivalTarget = 0) {
  let last = null;
  for (const tol of tiers) {
    const proj = projectStops(poly, stops, tol);
    const r = planFuel(routeMiles, proj, maxRange, startBurned, arrivalReserve, arrivalTarget);
...
function planBeyondGap(poly, routeMiles, stops, maxRange, gap, detourMax, arrivalReserve = 0, arrivalTarget = 0) {
  ...
  // The remainder still ends at the delivery, so the target applies to its
  // final stop exactly as it does on an ungapped run.
  const rest = planFuel(routeMiles - anchor.mile, shifted, maxRange, 0, arrivalReserve, arrivalTarget);
  const plan = [
