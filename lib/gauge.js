...
// What the driver can CHOOSE: 1/4, 3/8, 1/2 — and deliberately not 1/8, which
// v1.30.1 removed. Nobody wants to arrive on an eighth of a tank; offering it
// as a button presented a target no driver would pick.
//
// 1/8 is gone from the choices, NOT from the model. It is still RESERVE_TICKS,
// still the reserve every release has held back, and still what the planner
// uses when the driver has not raised it — which is why
// arrivalReserveMiles(RESERVE_TICKS) must stay 0, the standard reserve adding
// nothing on top of itself. test/gauge.test.js asserts RESERVE_TICKS is
// deliberately ABSENT from this array, so that its absence reads as a decision
// rather than an oversight.
const ARRIVAL_TICK_CHOICES = [2, 3, 4];
