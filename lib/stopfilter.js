 ...
 *   restaurants   Set of restaurant names (restaurantBrands). Empty means no
 *                 restaurant filter.
 *   rowRestaurants The restaurants at THIS row, as an array — passed in, like
 *                 rowCorridors, because the caller works them out once.
...
function stopPasses(row, criteria) {
  ...
  // Same shape as corridor: the stop passes if it has ANY picked restaurant,
  // so Popeyes and IHOP together return stops with either.
  if (c.restaurants && c.restaurants.size) {
    const has = c.rowRestaurants || [];
    if (!has.some(n => c.restaurants.has(n))) return false;
  }

  return true;
}

// The names a driver filters by, as the location master spells them — except
// where it spells ONE counter several ways. A driver picking Taco Bell means
// the Express and the Taco Bell & Pizza Hut Express too, and the card for each
// of those says "Taco Bell"; without these the Taco Bell pick would miss 10
// of its 22 stops. Anything not listed here is its own name.
const RESTAURANT_ALIASES = {
  'Taco Bell & Pizza Hut Express': ['Taco Bell', 'Pizza Hut'],
  'Taco Bell Express': ['Taco Bell'],
  'Pizza Hut Express': ['Pizza Hut'],
  "Miss J's Café": ["Miss J's"]
};

// One stop's [full service, quick service] (index.html's RESTAURANTS entry)
// as the names it can be filtered by. Quick service is a comma list; no name
// in the master has a comma of its own. Missing entry -> [].
function restaurantBrands(entry) {
  const [full, quick] = entry || ['', ''];
  const out = [];
  const add = n => { if (n && !out.includes(n)) out.push(n); };
  add(full);
  String(quick || '').split(',').map(n => n.trim()).filter(Boolean)
    .forEach(n => (RESTAURANT_ALIASES[n] || [n]).forEach(add));
  return out;
}

module.exports = { stopPasses, hasAmenCode, searchHaystack, restaurantBrands, RESTAURANT_ALIASES, COL };
